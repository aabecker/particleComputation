...
if nargin < 1  %TODO: why is this here?
    tileXY = [1 3];
    partXY=[2 2;2 3;2 4];
end
...
d = realmax*ones(part_width,1); %contains distances, init to maximum double
cnt = 1;
for i=max(partXYt(:,1)):-1:min(partXYt(:,1)) %run for loop from last row of the partXY to its minimum row
