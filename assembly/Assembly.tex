...
\begin{algorithmic}[1]
 \Procedure {BuildFactory}{$partXY$, $numCopies$}
 \Procedure {FindBuildPath}{$partXY$}
 \If{$false=IsPossible$}
 return
 \EndIf
 \EndProcedure
 \For{$i\leftarrow 2,size(sequenceXY)$}
\Procedure {factoryAddTile}{$partXYbuild$,$XYcoord$,$directions$,$partColoredArray$}
return factoryObstacleAdditionArray, partXYbuild
\EndProcedure
\Procedure {concatfactories}{$factoryLayoutArray$,$factoryObstacleadditionArray$}
return factoryLayoutArray
\EndProcedure
\EndFor
\Procedure{partFactorytest}{$factoryLayoutArray$}
\EndProcedure
\EndProcedure
\end{algorithmic}
 \end{algorithm} 
