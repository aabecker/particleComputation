...
\scriptsize
\caption{ \sc{BuildFactory}($partXY$, $numCopies$)}
%\Require $partXY$ is a 4-connected polyomino.  $numCopies$ is the number of parts desired.
\begin{algorithmic}[1]
% \Function{BuildFactory}{$partXY$, $numCopies$}
\State $\{ IsPossible,$directions$,$partColoredArray \}$  \leftarrow$ {\sc{FindBuildPath}}($part_{xy}$)
 \If{$false=IsPossible$}
 return
 \EndIf
% \EndFunction
 \For{$i\leftarrow 2,size(sequenceXY)$}
\State$\{ factoryAdditionArray, $partXYbuild\}$\leftarrow${\sc{factoryAddTile}}(\newline$partXYbuild$,$XYcoord$,$directions$,$partColoredArray$)
\State$\{ factoryLayoutArray\}\leftarrow${\sc{concatfactories}}(\newline$factoryLayoutArray$,$factoryObtacleadditionArray$)
\EndFor
\State{\sc{DisplayFactory}}($factoryLayoutArray$)
\end{algorithmic}
\end{algorithm} 
