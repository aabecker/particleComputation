...
%\begin{figure}
%\centering
%\subfloat[]{\includegraphics[width=3.1in]{fig1b2.pdf}\label{fig:fig1a}} 
%\caption{(a)A seven tile factory. Each particle is actuated simultaneously by the same global field. The factory is designed so each clockwise control input assembles another component.}
%\subfloat[]{\includegraphics[width=3.1in]{NewFig1b1.pdf}\label{fig:fig1b}}
%\caption{(b)A seven tile factory. Each particle is actuated simultaneously by the same global field. The factory is designed so each clockwise control input assembles another component.}
%\subfloat[]{\includegraphics[width=3.1in]{Fig1c2.pdf}\label{fig:fig1c}}
%\caption{(c) A seven tile factory. Each particle is actuated simultaneously by the same global field. The factory is designed so each clockwise control input assembles another component.}
%\label{fig:1ab} 
%\end{figure}
...
\centering
\subfloat[Factory to generate seven-tile polyomino at left, 0 commanded moves, 0 unit steps \label{cw_10}]{%
  \includegraphics[width=0.45\textwidth]{fig1b2.pdf}%
  }\par
\subfloat[Same factory, 18 commanded moves, ?? unit steps\label{cw_25}]{%
  \includegraphics[width=0.45\textwidth]{NewFig1b1.pdf}%
  }\par        
\subfloat[Parallel assembly with three factories, 28 commanded moves, ?? unit steps, three complete polyominoes\label{cw_50}]{%
  \includegraphics[width=0.45\textwidth]{Fig1c3.pdf}%
  }
...
\subsection{Control Swarms Using Only Global Signals}
