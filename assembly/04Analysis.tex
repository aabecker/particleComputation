...
%###############################################################
This section analyzes the travel distance in and space required for a factory and gives simulation results. Algorithms  \ref{alg:FindBuildPath} through \ref{alg:FactoryAddTile}  were coded in {\sc Matlab} and are available at \cite{Manzoor2017gitAssemply}.  
...
Once the first part hopper empties, the $n-1$ \emph{wind down}  cycles each produce a complete polyomino as each successive hopper empties.
 ...
 There are two results, the \emph{construction distance}, its the maximum distance required to assemble a single polyomino from scratch, and
 the \emph{cycle distance}, the maximum distance required during production cycles to advance all partial assemblies one cycle.
...
\end{overpic}
\vspace{-2em}
\caption{\label{fig:timeplot}Cycle distance plotted against number of tiles $n$.  The cycle distance is the sum of distances to move during the $r,d,l,u$ moves each cycle. Cycle distance increases linearly and is upper bounded by row parts and lower bounded by column parts.  Total construction distance for a particle is $n\cdot$cycle distance.  
}
\end{figure}
%###############################################################
...
%\end{align}
%\begin{align}
width_{hopper+delays}&=w+(2\left \lceil{ \frac{i}{2}}\right \rceil+8),  i \geq 2 \label{eq:2} \end{align}
\begin{align}
width(i)=&~width_{hopper+delays} +\nonumber\\
&\begin{cases}
