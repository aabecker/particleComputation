...
%\scriptsize 
\caption{\sc {FindBuildPath}($\mathbf{P})$}
$\mathbf{P}$ is the $x,y$ coordinates of a 4-connected polyomino. 
Returns $ \mathbf{C} $, $ \mathbf{c} $ and $\mathbf{m}$ where $ \mathbf{C} $ contains sequence of polyomino coordinates, $ \mathbf{c} $ is an array of color labels and $\mathbf{m}$ is an array of directions.
\begin{algorithmic}[3]
%\For{$m=1:partXY(:,1)$}
...
%\scriptsize
\caption{\sc {FactoryAddTile}(($n,\mathbf{b}, \mathbf{m}_i,\mathbf{C}_i, \mathbf{c}_i)$)}
\begin{algorithmic}[2]
\State$\{ \mathbf{hopper}\}\leftarrow${\sc{Hopper}}$(\mathbf{c}_i),n,4)$
\If{$\mathbf{m}_i= d$}
%$maxpartx=max(partXY(:,2));$
\If{$\mathbf{C}_i(1,2)<=\sc{max}\mathbf{b}(:,2)$}
...
\If{$\mathbf{m}_i= l$}
%$maxpartx=max(partXY(:,2));$
\If{$\mathbf{C}_i(1,1)<=\sc{max}\mathbf{b}(:,1)$}
\State$\{\mathbf{A},\mathbf{b}\}\leftarrow${\sc{leftdir}}$(\mathbf{hopper},\mathbf{b},\mathbf{C}_i)$
\Else
...
\If{$\mathbf{m}_i= u$}
%$maxpartx=max(partXY(:,2));$
\If{$\mathbf{C}_i(1,2)<=\sc{min}\mathbf{b}(:,2)$}
\State$\{\mathbf{A},\mathbf{b}\}\leftarrow${\sc{updir}}$(\mathbf{hopper},\mathbf{b},\mathbf{C}_i)$
\Else
...
\If{$\mathbf{m}_i= r$}
%$maxpartx=max(partXY(:,2));$
\If{$\mathbf{C}_i(1,2)<=\sc{min}\mathbf{b}(:,1)$}
\State$\{\mathbf{A},\mathbf{b}\}\leftarrow${\sc{rightdir}}$(\mathbf{hopper},\mathbf{b},\mathbf{C}_i)$
\Else
\State$\{\mathbf{A},\mathbf{b}\}\leftarrow${\sc{downdir}}$(\mathbf{hopper},\mathbf{b},\mathbf{C}_i)$
\EndIf
