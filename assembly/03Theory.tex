...
This paper uses \emph{cycles} of movement commands in the order $\langle r,d,l,u \rangle$. We assume the area of $W$ is finite and issue each command long enough for the particles to reach their maximum extent.
%\end{enumerate}
...
Given a polyomino, Alg.~\ref{alg:FindBuildPath} determines if the polyomino can be built by adding one component at a time.
 ...
 Each new tile must have a straight-line path to its goal position in the polyomino that does not collide with any other particle, does not slide past an opposite specie of tile, and terminates in a mating configuration with an opposite specie tile.
