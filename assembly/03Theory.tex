...
As in many robotics problem, the inverse problem of deconstruction is easier than the forward problem of construction.  
...
Alg.~\ref{alg:FindBuildPath}  first assigns each tile in the polynomial a color, then calls the recursive function {\sc {Decompose}}$(\mathbf{P},\mathbf{c})$, which returns either a build order of polyomino coordinates and the directions to build, or an empty list if the part cannot be constructed.  
{\sc {Decompose}} calls the function {\sc {Erode}}.  {\sc {Erode}} first counts the number of components in the 8-connected freespace.  If there is more than one connected component, the polynomial contains loops.  
...
For each tile, the function 
 {\sc{FactoryAddTile}}$(n,\mathbf{b}, m,C, c,w)$ described in  Alg.~\ref{alg:FactoryAddTile}
is called to generates an obstacle configuration $\mathbf{A}$.
$\mathbf{A}$  forms a hopper that releases a particle each iteration and a chamber that temporarily holds the partially-assembled polyomino $\mathbf{b}$ and guides the new particle $C$ to the correct mating position. A 24-tile factory is shown in  Fig.~\ref{alg:FactoryAddTile}.
