...
G.th = linspace(0,2*pi,50);
for hi = 1: numRobots
    %G.rLines(hi) = line(RobotPts(hi,1),RobotPts(hi,2),'Color',0.8*colors(RobotPts(hi,4),:),'linewidth',3);
    G.rLines(hi) =  patch([NaN,RobotPts(hi,1)],[NaN,RobotPts(hi,2)],0.8*colors(RobotPts(hi,4),:),'linewidth',3,'FaceAlpha',0,'EdgeAlpha',0.8,'EdgeColor',0.8*colors(RobotPts(hi,4),:));
    ...
    G.hRobots(hi) = patch(RobotPts(hi,1)+G.rad*cos(G.th),RobotPts(hi,2)+G.rad*sin(G.th),colors(RobotPts(hi,4),:),'linewidth',2,'FaceAlpha',.95,'EdgeAlpha',.95,'EdgeColor',0.3*colors(RobotPts(hi,4),:));
...
vecColor = [0,0,0.5];
G.VectorLineW = patch(G.VectorLineX,G.VectorLineY,vecColor,'linewidth',4,'FaceAlpha',0.8,'EdgeAlpha',0.4,'EdgeColor',vecColor);
...
if PLAY_GAME
    ...
    for i = 1:numRobots
        ...
        G.hGoals(i) = patch(G.goals(i,1)+G.radG*cos(G.th),G.goals(i,2)+G.radG*sin(G.th),0.7*colors(i,:),'linewidth',4,'FaceAlpha',0,'EdgeAlpha',0.8,'EdgeColor',0.7*colors(i,:));
        %rectangle('Position',[G.goals(i,1)-G.radG/2,G.goals(i,2)-G.radG/2,G.radG,G.radG],'linewidth',4,'Curvature',[1,1],'FaceColor','none','EdgeColor',0.7*colors(i,:),'EdgeAlpha',0.4);
    end
...
if MAKE_MOVIE
    ...
    for i = 1:numRobots
        ...
        set(G.hGoals(i),'linewidth',1,'FaceAlpha',0,'EdgeAlpha',0,'EdgeColor',colors(i,:));
    ...
    for i = 1:numRobots
        ...
        for hi = [0,1,2,3,4,5,6]
            set(G.hGoals(i),'linewidth',4+hi,'FaceAlpha',0,'EdgeAlpha',0.8,'EdgeColor',min(1,(0.7+hi*.1))*colors(i,:));
            updateDrawing;
        end
    end
    for hi = [5,4,3,2]
