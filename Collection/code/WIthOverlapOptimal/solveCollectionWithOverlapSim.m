function solveCollectionWithOverlapSim
...
G.ptail = 1;
G.mxSize = 1000;
G.LeafRobots = zeros(G.mxSize,numel(G.robvec));
G.LeafPath = zeros(G.mxSize,1);
G.LeafPrev = zeros(G.mxSize,1);
%TODO: try bit packing: d =bi2de(b)
G.LeafRobots(phead,:) = G.robvec;
while phead <= G.ptail
    rvec = G.LeafRobots(phead,:);
...
%%%%%%%%%
    function isFinished = expandLeaf(mv,rvec,phead)
        ...
        if sum(rvecT)==1 %     return success!
            ...
            display( shortpath)
            %phead = G.ptail+1;
            save([datestr(clock,30),'.mat'], 'shortpath', 'phead','G');
            isFinished = true;
        elseif sum(ismember(G.LeafRobots,rvecT,'rows')) == 0
            G.ptail = G.ptail+1;
