function solveCollectionWithOverlapBinSorted
%  TODO: save the files each time it adds more positions.
% Also, change the dimension slightly with each.
%
% difference from solveCollectionWithOverlapBin is that it also maintains a
% sorted list of leafs that is used during expansion.
% New plan is to rerun the algorithm, but eliminate all rotations.
%
...
%%%%%%%%%
    ...
    function isNew = IsNewLeaf(brvecT)
        ...
        %%%%OLD WORKING CODE
        %         for i = 1:G.ptail
        %             if Leafsorted(i) > brvecT
        %                 Leafsorted(1:G.ptail+1) = [Leafsorted(1:i-1); brvecT; Leafsorted(i:G.ptail)];
        %                 return;
        %             end
        %             if Leafsorted(i) == brvecT
        %                 isNew = false;
